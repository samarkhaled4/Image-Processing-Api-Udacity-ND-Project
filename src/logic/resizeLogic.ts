...
const displayImage = async (
    ...
    if (fs.existsSync(myImg)) {
        ...
        if (!imageWidth && !imageHeight) return myImg
        else {
            ...
            if (!imageHeight) {
                const newImage = path.join(
                    resizeFolder,
                    `${imageName}_width-${imageWidth}.jpg`
                )
            ...
            //resize image with only height
                ...
                const newImage = path.join(
                    resizeFolder,
                    `${imageName}_height-${imageHeight}.jpg`
                )
                if (!fs.existsSync(newImage)) {
            ...
            //resize image with width and height
                ...
                const newImage = path.join(
                    resizeFolder,
                    `${imageName}_width-${imageWidth}_height-${imageHeight}.jpg`
                )
                if (!fs.existsSync(newImage)) {
                    await sharp(myImg)
                        .resize({ width, height })
